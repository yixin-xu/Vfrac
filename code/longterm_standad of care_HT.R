...
# HT: Yixin, please have separate data, code, and results directories in the GitHub repository.
# HT: Notes that I changed shotterm to shortterm everywhere.


# read excel file (T5 is sarash's inc Q and inc C)
# HT: Yixin, I've commented out the setwd() line as you don't need it if you're using a project
...
shortterm=shortterm_raw%>% drop_na(69) # Change shortterm to shortterm_raw then T4 is shortterm
...
gp_xvfracid = shortterm$xvfracid[shortterm$mrbackpainbas == 1]

# vfyncode = Does this person have an osteoporotic vertebral fracture
# change "Yes" and "No" to 1 and 0
shortterm$vfyncode[shortterm$vfyncode == "Yes"] = 1 # Had fracture
shortterm$vfyncode[shortterm$vfyncode == "No"] = 0 # Did not have fracture
...
for(i in 1:n_samples){
  ...
  for(a in gp_xvfracid){
    ...
    if(a %in% randomsample & shortterm$vfyncode[shortterm$xvfracid == a] == 1){
      random_QALYs[match(a,gp_xvfracid),i+1] = shortterm$feq5d_score[shortterm$xvfracid == a]*0.25 + PSA_results[i,"inc Q"]} 
    
    if(a %in% randomsample & shortterm$vfyncode[shortterm$xvfracid == a] == 1){
      random_costs[match(a,gp_xvfracid),i+1] = shortterm$fcosts[shortterm$xvfracid == a] + PSA_results[i, "inc C"]} 
  }
}
...
leftjoin_qalys = left_join(shortterm, random_QALYs, by ='xvfracid')
...
for(i in 1:dim(bootstrap_samples)[1]) {
  bootstrap_samples[i, ] <- sample(1:sum(!is.element(shortterm$xvfracid, random_QALYs$gp_xvfracid)), size = n_samples)
}
...
for(i in Total_samples_QALYs$leftjoin_qalys...2.){
  ...
  if(i %in% random_QALYs$gp_xvfracid == FALSE){
    ...
    Total_samples_QALYs[match(i,Total_samples_QALYs$leftjoin_qalys...2.),2:(n_samples+1)] = 
      shortterm$beq5d_score[bootstrap_samples[which(Total_samples_QALYs$leftjoin_qalys...2.==i), ]]*0.25}
...
#  Total_samples_QALYs[a,is.na(Total_samples_QALYs[a,])] =   shortterm$beq5d_score[bootstrap_samples[a,is.na(Total_samples_QALYs[a, -1])]]
...
for(a in nalist){
  Total_samples_QALYs[a,is.na(Total_samples_QALYs[a,])] = shortterm$beq5d_score[a]*0.25
}

# Add the IDs to the costs data
random_costs$xvfracid = random_costs$gp_xvfracid
leftjoin_costs = left_join(shortterm, random_costs, by ='xvfracid')
...
for(i in Total_samples_costs$leftjoin_costs...2.){
  ...
  if(i %in% random_costs$gp_xvfracid == FALSE){
    ...
    Total_samples_costs[match(i,Total_samples_costs$leftjoin_costs...2.),2:n_samples+1] = 
      shortterm$bcosts[bootstrap_samples[which(Total_samples_costs$leftjoin_costs...2.==i), ]]}
...
for(a in nalist){
  Total_samples_costs[a,is.na(Total_samples_costs[a,])] =  shortterm$bcosts[a]
}
...
### NET BENEFIT
nb_standad_of_care = data.frame(Total_samples_QALYs_mean*2000 - Total_samples_costs_mean)

# HT: Also need to export bootstrap_samples so the same samples can be used in longterm_Vfrac.R
write.csv(nb_standad_of_care, 'nb_standad_of_care.csv')
