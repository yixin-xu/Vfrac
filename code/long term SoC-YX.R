...
### QALYs abd costs for patients had gp consultation
  ...
  for(a in gp_xvfracid){
    ...
    if(a %in% randomsample){
      ...
      if(patient_infor$vfyncode[patient_infor$xvfracid == a] == "Yes"){
        ...
        random_QALYs[match(a,random_QALYs$`patient_infor$xvfracid`),i+1] = patient_infor$feq5d_score[patient_infor$xvfracid == a]*0.25 + PSA_results[i,5]} 
      else{
        # No fracture so no additional qalys
        random_QALYs[match(a,random_QALYs$`patient_infor$xvfracid`),i+1] = patient_infor$feq5d_score[patient_infor$xvfracid == a]*0.25}
    }
      ...
      random_QALYs[match(a,random_QALYs$`patient_infor$xvfracid`),i+1] = patient_infor$beq5d_score[patient_infor$xvfracid == a]*0.25 + PSA_results[i,3]
    } else{
      # No fracture so no additional qalys
      random_QALYs[match(a,random_QALYs$`patient_infor$xvfracid`),i+1] = patient_infor$beq5d_score[patient_infor$xvfracid == a]*0.25}
   ...
   if(a %in% randomsample){
    ...
    if(patient_infor$vfyncode[patient_infor$xvfracid == a] == "Yes"){
      # Have a fracture then get treatment
      random_costs[match(a,random_costs$`patient_infor$xvfracid`),i+1] = patient_infor$fcosts[patient_infor$xvfracid == a]+ PSA_results[i,4]} 
    else{
      # No fracture so no additional costs
      random_costs[match(a,random_costs$`patient_infor$xvfracid`),i+1] = patient_infor$fcosts[patient_infor$xvfracid == a]}
  }
  else if(patient_infor$vfyncode[patient_infor$xvfracid == a] == "Yes"){
    # Have a fracture but don't get treatment so add no treatment costs
    random_costs[match(a,random_costs$`patient_infor$xvfracid`),i+1] = patient_infor$bcosts[patient_infor$xvfracid == a] + PSA_results[i,2]
  } else{
    # No fracture so no additional costs
    random_costs[match(a,random_costs$`patient_infor$xvfracid`),i+1] = patient_infor$bcosts[patient_infor$xvfracid == a]}
}
}
...
for(i in 1:nrow(patient_infor)){
  ...
  if(!(patient_infor$xvfracid[i]%in%gp_xvfracid) ){
