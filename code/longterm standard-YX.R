...
shortterm_qalys = read.csv('results/shortterm-standard-qalys.csv')
shortterm_costs = read.csv('results/shortterm-standard-costs.csv')

st_qalys_mean = rowMeans(shortterm_qalys[,2:1001])
st_costs_mean = rowMeans(shortterm_costs[,2:1001])
...
for(i in Total_samples_QALYs$leftjoin_qalys...2.){
  ...
  if(i %in% random_QALYs$gp_xvfracid == FALSE){
    ...
    Total_samples_QALYs[match(i,Total_samples_QALYs$leftjoin_qalys...2.),2:(n_samples+1)] = 
      st_qalys_mean[bootstrap_samples[which(Total_samples_QALYs$leftjoin_qalys...2.==i), ]]}
...
for(a in nalist){
  Total_samples_QALYs[a,is.na(Total_samples_QALYs[a,])] = st_qalys_mean[bootstrap_samples[a,is.na(Total_samples_QALYs[a, -1])]]
}
...
nb_mean_col = colMeans(nb_standard_of_care)
mean(nb_mean_col)
